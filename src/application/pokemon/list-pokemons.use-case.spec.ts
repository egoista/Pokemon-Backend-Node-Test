...
describe('ListPokemonsUseCase', () => {
  ...
  it('should validate sortBy and sortOrder', async () => {
    await expect(useCase.execute({ sortBy: 'invalid' })).rejects.toThrow(
      ValidationError,
    );
    await expect(useCase.execute({ sortOrder: 'invalid' })).rejects.toThrow(
      ValidationError,
    );
  });

  it('should validate query input presence', async () => {
    await expect(useCase.execute(null as unknown as any)).rejects.toThrow(
      ValidationError,
    );
  ...
  it('should validate type and name types', async () => {
    await expect(
      useCase.execute({ type: 123 as unknown as string }),
    ).rejects.toThrow(ValidationError);
    await expect(
      useCase.execute({ name: 123 as unknown as string }),
    ).rejects.toThrow(ValidationError);
  });
});
