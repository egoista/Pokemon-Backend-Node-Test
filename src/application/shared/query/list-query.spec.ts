...
describe('normalizeListQuery', () => {
  ...
  it('throws when sortBy is not allowed', () => {
    expect(() =>
      normalizeListQuery({ sortBy: 'created_at' }, baseOpts),
    ).toThrow(ValidationError);
  });

  it('throws when sortOrder is invalid', () => {
    expect(() =>
      normalizeListQuery({ sortOrder: 'sideways' }, baseOpts),
    ).toThrow(ValidationError);
  });

  it('throws when input is missing', () => {
    expect(() => normalizeListQuery(null as unknown as any, baseOpts)).toThrow(
      ValidationError,
    );
  });
  ...
  it('throws when page is not a number', () => {
    expect(() =>
      normalizeListQuery({ page: '1' as unknown as number }, baseOpts),
    ).toThrow(ValidationError);
  });

  it('throws when limit is not a number', () => {
    expect(() =>
      normalizeListQuery({ limit: '10' as unknown as number }, baseOpts),
    ).toThrow(ValidationError);
  });

  it('throws when sortBy is not a string', () => {
    expect(() =>
      normalizeListQuery({ sortBy: 1 as unknown as string }, baseOpts),
    ).toThrow(ValidationError);
  });

  it('throws when sortOrder is not a string', () => {
    expect(() =>
      normalizeListQuery({ sortOrder: 1 as unknown as string }, baseOpts),
    ).toThrow(ValidationError);
  });

  it('throws when page is not a positive integer', () => {
    expect(() => normalizeListQuery({ page: 1.5 }, baseOpts)).toThrow(
      ValidationError,
    );
  });

  it('throws when limit is not a positive integer', () => {
    expect(() => normalizeListQuery({ limit: 0 }, baseOpts)).toThrow(
      ValidationError,
    );
  });
});
