...
describe('AppModule', () => {
    ...
    it('formats GraphQL errors with sanitized payload', async () => {
        ...
        const result = gqlOptions.formatError({ message: 'Boom' } as GraphQLFormattedError);
    ...
    it('enables landing page plugin outside test environment', async () => {
        ...
        const imports = Reflect.getMetadata(MODULE_METADATA.IMPORTS, module.AppModule) as any[];
        const graphqlModule = imports.find((entry) => entry?.module?.name === 'GraphQLModule');
        ...
        const definitionsPath = String(gqlOptions?.definitions?.path ?? '').replace(/\\/g, '/');

        expect(Array.isArray(gqlOptions?.plugins)).toBe(true);
        expect(gqlOptions?.plugins?.length).toBe(1);
        expect(definitionsPath).toContain('/src/infrastructure/graphql/generated/graphql.ts');
    });
    ...
    it('imports TypeOrmModule when repository is typeorm', async () => {
        ...
        const module = await import('./app.module');
        const imports = Reflect.getMetadata(MODULE_METADATA.IMPORTS, module.AppModule) as any[];
        const flattenedImports = imports.flatMap((entry: any) => Array.isArray(entry) ? entry : [entry]);
        const hasTypeOrmImport = flattenedImports.some((entry) => entry?.module?.name === 'TypeOrmModule');
        const hasPrismaImport = flattenedImports.some((entry) => entry?.name === 'PrismaModule');
    ...
    it('uses throttler env overrides when provided', async () => {
        ...
        process.env.RATE_LIMIT_MAX_REQUESTS = '42';
        const imports = Reflect.getMetadata(MODULE_METADATA.IMPORTS, module.AppModule) as any[];
        const throttlerModule = imports.find((entry) => entry?.module?.name === 'ThrottlerModule');
