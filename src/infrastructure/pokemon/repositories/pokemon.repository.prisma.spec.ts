...
describe('PokemonRepositoryPrisma', () => {
  ...
  beforeEach(() => {
    prisma = {
      ...
      $transaction: jest
        .fn()
        .mockImplementation((calls: Promise<unknown>[]) => Promise.all(calls)),
    };

    repository = new PokemonRepositoryPrisma(
    ...
    expect(prisma.pokemon.update).toHaveBeenCalledWith({
      ...
      data: {
        ...
        types: {
          set: [],
          connectOrCreate: [
            { where: { name: 'Water' }, create: { name: 'Water' } },
          ],
        },
      },
      include: { types: true },
    ...
    expect(prisma.pokemon.upsert).toHaveBeenCalledWith({
      ...
      update: {
        ...
        types: {
          set: [],
          connectOrCreate: [
            { where: { name: 'Fire' }, create: { name: 'Fire' } },
          ],
        },
      },
      create: {
