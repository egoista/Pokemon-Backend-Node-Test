...
describe('TypeORM entities metadata', () => {
  it('registers columns and relations for pokemon and type entities', () => {
    ...
    expect(
      pokemonRelations.some((relation) => relation.propertyName === 'types'),
    ).toBe(true);
    expect(
      typeRelations.some((relation) => relation.propertyName === 'pokemons'),
    ).toBe(true);
    expect(
      pokemonColumns.some((column) => column.propertyName === 'created_at'),
    ).toBe(true);
    expect(
      typeColumns.some((column) => column.propertyName === 'created_at'),
    ).toBe(true);
  });
